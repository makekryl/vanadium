      ...
      [&](const core::ast::Node* n) -> bool {
        if (overlaps(requested_range, n->nrange)) {
          if (n->nkind == core::ast::NodeKind::Ident) {
            const auto* ident = n->As<core::ast::nodes::Ident>();
            ...
            if (const auto* sym = core::checker::ResolveExprSymbol(&file, scope, ident); sym) {
              ...
              std::tie(w.type, w.modifiers) = [&] -> Fu {
                if (sym->Flags() & core::semantic::SymbolFlags::kTemplate) {
                  return {lsp::SemanticTokenTypes::kVariable, (ident->parent->nkind == core::ast::NodeKind::FuncDecl
                                                                   ? lsp::SemanticTokenModifiers::kDeclaration
                                                                   : lsp::SemanticTokenModifiers::kUnset) |
                                                                  lsp::SemanticTokenModifiers::kReadonly};
                }
                if (sym->Flags() & core::semantic::SymbolFlags::kFunction) {
                  return {lsp::SemanticTokenTypes::kFunction, (ident->parent->nkind == core::ast::NodeKind::FuncDecl
                                                                   ? lsp::SemanticTokenModifiers::kDeclaration
                                                                   : lsp::SemanticTokenModifiers::kUnset)};
                }
                ...
                if (sym->Flags() & core::semantic::SymbolFlags::kArgument) {
                  return {lsp::SemanticTokenTypes::kParameter, (ident->parent->nkind == core::ast::NodeKind::FormalPar
                                                                    ? lsp::SemanticTokenModifiers::kDeclaration
                                                                    : lsp::SemanticTokenModifiers::kUnset)};
                }
