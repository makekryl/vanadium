...
extern "C" {
void optional_ctor(void*);
void optional_dtor(void*);
}

const vrt_typeinfo_t vrt_optional_typeinfo{
    .name = "vrt_optional",
    .kind = vrt_typekind_e::kOptionalMember,
    .size = sizeof(vrt_optional_t),

    .members = nullptr,

    .construct = optional_ctor,
    .destruct = optional_dtor,
};

void optional_ctor(void*) {
  vrt_panic("vrt_optional_ctor should not be called");
}
void optional_dtor(void* p) {
